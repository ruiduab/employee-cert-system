#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
高效岗位去重脚本 - 使用分组策略
避免O(n²)的全量比较，大幅加速
"""

import json
import re
from difflib import SequenceMatcher
from collections import defaultdict
from pathlib import Path

def normalize_position(pos):
    """标准化职位名称用于比较"""
    pos = pos.lower().strip()
    # 移除特殊字符
    pos = re.sub(r'[^\w\s\u4e00-\u9fff]', '', pos)
    # 规范空格
    pos = re.sub(r'\s+', ' ', pos)
    return pos

def get_tokens(pos):
    """提取关键词用于分组"""
    tokens = set()
    words = pos.split()

    # 添加单个词汇
    for w in words:
        if len(w) > 2:  # 只要长度>2的词
            tokens.add(w)

    # 添加前缀（用于匹配相似的岗位）
    for w in words:
        if len(w) >= 3:
            tokens.add(w[:3])

    return tokens

def similarity_score(s1, s2):
    """计算相似度"""
    if s1 == s2:
        return 1.0

    max_len = max(len(s1), len(s2))
    if max_len == 0:
        return 1.0

    # 使用SequenceMatcher计算相似度
    score = SequenceMatcher(None, s1, s2).ratio()
    return score

def deduplicate_by_grouping(positions, threshold=0.85):
    """
    使用分组策略进行高效去重
    策略：先按关键词分组，再在组内比较
    """
    print(f"\n🔍 开始智能去重（阈值: {threshold})...")
    print(f"   总岗位数: {len(positions)}\n")

    # 第一步：建立关键词索引
    print("📑 第一步：建立关键词索引...")
    keyword_index = defaultdict(list)

    for i, pos in enumerate(positions):
        if i % 10000 == 0:
            progress = i / len(positions) * 100
            print(f"  [{i:6d}/{len(positions)}] {progress:5.1f}% - 已索引")

        # 提取中文和英文名称
        pos_cn = pos.get('name_cn', '').lower() if pos.get('name_cn') else ''
        pos_en = pos.get('name_en', '').lower() if pos.get('name_en') else ''

        # 获取规范化的名称
        norm_cn = normalize_position(pos_cn) if pos_cn else ''
        norm_en = normalize_position(pos_en) if pos_en else ''

        # 提取所有关键词
        tokens = set()
        if norm_cn:
            tokens.update(get_tokens(norm_cn))
        if norm_en:
            tokens.update(get_tokens(norm_en))

        # 加入索引
        for token in tokens:
            keyword_index[token].append(i)

    print(f"✅ 索引完成：{len(keyword_index)} 个关键词\n")

    # 第二步：在共享关键词的岗位中寻找重复
    print("🔗 第二步：寻找相似岗位组...")

    duplicates = defaultdict(list)  # 候选重复的映射
    processed = set()

    for idx, pos in enumerate(positions):
        if idx % 10000 == 0:
            progress = idx / len(positions) * 100
            print(f"  [{idx:6d}/{len(positions)}] {progress:5.1f}% - 已处理")

        if idx in processed:
            continue

        pos_cn = pos.get('name_cn', '')
        pos_en = pos.get('name_en', '')
        norm_cn = normalize_position(pos_cn) if pos_cn else ''
        norm_en = normalize_position(pos_en) if pos_en else ''

        # 获取所有可能的候选
        candidates = set()
        tokens = set()
        if norm_cn:
            tokens.update(get_tokens(norm_cn))
        if norm_en:
            tokens.update(get_tokens(norm_en))

        for token in tokens:
            candidates.update(keyword_index[token])

        candidates.discard(idx)  # 移除自己

        # 与候选比较
        similar_group = [idx]
        for cand_idx in candidates:
            if cand_idx in processed or cand_idx == idx:
                continue

            cand_pos = positions[cand_idx]
            cand_cn = cand_pos.get('name_cn', '')
            cand_en = cand_pos.get('name_en', '')
            norm_cand_cn = normalize_position(cand_cn) if cand_cn else ''
            norm_cand_en = normalize_position(cand_en) if cand_en else ''

            # 比较相似度
            max_score = 0
            if norm_cn and norm_cand_cn:
                cn_score = similarity_score(norm_cn, norm_cand_cn)
                max_score = max(max_score, cn_score)
            if norm_en and norm_cand_en:
                en_score = similarity_score(norm_en, norm_cand_en)
                max_score = max(max_score, en_score)
            # 交叉比较
            if norm_cn and norm_cand_en:
                cross_score = similarity_score(norm_cn, norm_cand_en)
                max_score = max(max_score, cross_score * 0.8)  # 打个折扣

            if max_score >= threshold:
                similar_group.append(cand_idx)
                processed.add(cand_idx)

        if len(similar_group) > 1:
            duplicates[idx] = similar_group

        processed.add(idx)

    print(f"✅ 找到 {len(duplicates)} 个相似岗位组\n")

    return duplicates, positions

def merge_positions(positions, duplicates):
    """合并重复岗位"""
    print(f"📋 开始合并岗位...\n")

    merged = []
    removed_indices = set()
    merge_log = []

    for idx, pos in enumerate(positions):
        if idx in removed_indices:
            continue

        if idx in duplicates:
            # 这是一个主岗位，有重复的
            group = duplicates[idx]
            for dup_idx in group[1:]:
                removed_indices.add(dup_idx)

            # 选择最佳版本
            best_pos = pos
            removed_names = []

            for dup_idx in group[1:]:
                dup_pos = positions[dup_idx]
                # 优先保留原始中文数据
                if (best_pos.get('source') != 'chinese_original' and
                    dup_pos.get('source') == 'chinese_original'):
                    removed_names.append(best_pos.get('name_cn') or best_pos.get('name_en'))
                    best_pos = dup_pos
                elif ('name_cn' not in best_pos or not best_pos.get('name_cn')) and \
                     'name_cn' in dup_pos and dup_pos.get('name_cn'):
                    removed_names.append(best_pos.get('name_cn') or best_pos.get('name_en'))
                    best_pos = dup_pos
                else:
                    removed_names.append(dup_pos.get('name_cn') or dup_pos.get('name_en'))

            merged.append(best_pos)
            merge_log.append({
                'kept': best_pos.get('name_cn') or best_pos.get('name_en'),
                'merged_count': len(group) - 1,
                'removed_names': removed_names
            })
        else:
            # 不是重复的，直接保留
            merged.append(pos)

    return merged, merge_log

def main():
    print("=" * 90)
    print("⚡ 高效岗位库去重工具")
    print("=" * 90)

    # 加载数据
    json_file = '/Users/zhangrui/Claude工作/企业职工认证/positions-bilingual.json'
    print(f"\n📂 加载岗位数据: {json_file}")

    with open(json_file, 'r', encoding='utf-8') as f:
        data = json.load(f)

    positions = data['positions']
    print(f"✅ 已加载 {len(positions)} 个岗位\n")

    # 选择相似度阈值
    print("🎯 去重模式:")
    print("   1. 严格去重 (0.95) - 只合并极相似的岗位")
    print("   2. 标准去重 (0.85) - 合并很相似的岗位 ✅ 推荐")
    print("   3. 宽松去重 (0.75) - 合并相似的岗位\n")

    choice = input("选择 (1/2/3, 默认2): ").strip() or '2'
    thresholds = {'1': 0.95, '2': 0.85, '3': 0.75}
    threshold = thresholds.get(choice, 0.85)

    # 执行去重
    duplicates, positions = deduplicate_by_grouping(positions, threshold)

    if not duplicates:
        print(f"\n✅ 未找到重复岗位！库已经很干净。")
        return

    total_merged = sum(len(group) - 1 for group in duplicates.values())
    print(f"✅ 相似岗位组: {len(duplicates)}")
    print(f"   可合并的重复: {total_merged} 个\n")

    # 显示示例
    print("📋 合并示例 (前15个):")
    print(f"{'#':<4} {'保留岗位':<35} {'合并数':<4} {'移除的岗位':<40}")
    print("─" * 90)

    for i, (idx, group) in enumerate(list(duplicates.items())[:15], 1):
        pos = positions[idx]
        kept_name = (pos.get('name_cn') or pos.get('name_en', '?'))[:33]
        merged_count = len(group) - 1

        removed_names = []
        for dup_idx in group[1:]:
            dup_pos = positions[dup_idx]
            removed_names.append((dup_pos.get('name_cn') or dup_pos.get('name_en'))[:15])

        removed_str = ', '.join(removed_names[:2])
        if len(removed_names) > 2:
            removed_str += f" +{len(removed_names)-2}"

        print(f"{i:<4} {kept_name:<35} {merged_count:<4} {removed_str:<40}")

    if len(duplicates) > 15:
        print(f"{'...':<4} {'... 还有' + str(len(duplicates)-15) + '个组 ...':<35}")

    # 确认合并
    confirm = input(f"\n继续合并？(y/n, 默认n): ").strip().lower()
    if confirm != 'y':
        print("取消合并")
        return

    # 执行合并
    merged_positions, merge_log = merge_positions(positions, duplicates)

    print(f"\n✅ 合并完成!")
    print(f"   原始数量: {len(positions):,}")
    print(f"   合并后: {len(merged_positions):,}")
    print(f"   减少: {len(positions) - len(merged_positions):,} 个 ({(len(positions) - len(merged_positions)) / len(positions) * 100:.2f}%)")

    # 保存结果
    output_file = '/Users/zhangrui/Claude工作/企业职工认证/positions-deduplicated.json'
    output_data = {
        'total': len(merged_positions),
        'original_total': len(positions),
        'deduplicated': len(positions) - len(merged_positions),
        'description': '去重后的岗位库',
        'positions': merged_positions
    }

    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(output_data, f, ensure_ascii=False, indent=2)

    print(f"\n📝 保存JSON...")
    print(f"✅ 已保存: {output_file}")
    print(f"   大小: {len(json.dumps(output_data)) / 1024 / 1024:.1f} MB")

    # 保存合并报告
    report_file = '/Users/zhangrui/Claude工作/企业职工认证/DEDUPLICATION_REPORT.md'
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 岗位库去重报告\n\n")
        f.write(f"**处理时间**: 2026-04-15\n")
        f.write(f"**去重方式**: 高效分组去重\n")
        f.write(f"**相似度阈值**: {threshold}\n\n")
        f.write(f"## 统计结果\n\n")
        f.write(f"- 原始岗位数: {len(positions):,}\n")
        f.write(f"- 去重后: {len(merged_positions):,}\n")
        f.write(f"- 合并数量: {len(positions) - len(merged_positions):,} ({(len(positions) - len(merged_positions)) / len(positions) * 100:.2f}%)\n")
        f.write(f"- 相似岗位组: {len(duplicates)}\n\n")

        f.write(f"## 去重示例 (前20个)\n\n")
        f.write(f"| # | 保留岗位 | 合并数 | 移除的岗位 |\n")
        f.write(f"|---|----------|--------|------------|\n")

        for i, log in enumerate(merge_log[:20], 1):
            kept = log['kept'][:25]
            merged_count = log['merged_count']
            removed = ', '.join(log['removed_names'][:2])
            if len(log['removed_names']) > 2:
                removed += f" +{len(log['removed_names'])-2}"
            f.write(f"| {i} | {kept} | {merged_count} | {removed} |\n")

        if len(merge_log) > 20:
            f.write(f"\n... 还有 {len(merge_log)-20} 个合并操作\n")

    print(f"\n📊 报告已保存: {report_file}")

    print(f"\n{'='*90}")
    print(f"✨ 去重完成！")
    print(f"   下一步：用新的去重数据库更新SQL")
    print(f"{'='*90}\n")

if __name__ == '__main__':
    main()
